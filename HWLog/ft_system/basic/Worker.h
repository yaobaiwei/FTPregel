...
public:
	...
	void active_compute() {
		...
		rwthread = new thread(&MessageBuffer<VertexT>::local_save, message_buffer); //*** FT-change-2: to log msgs locally & asynchronously
	}
	...
	void load_CP(string input_path)
	{
		...
		//prepare msgbuf
		message_buffer->init(vertexes);//FT-change-2: only a new spawnee will call this
		if(global_CP_step > 0)
	...
	void write_CP(string input_path)//write cpx, where x>0
	{
		...
		//to remove locally logged msgs
		fs = getlocalFS();
		while(hdfsExists(fs, "/tmp/ftpregel") == 0) hdfsDelete(fs, "/tmp/ftpregel"); //*** FT-change-2
		hdfsDisconnect(fs);
		worker_barrier(); //*** FT-change: to guarantee that "/tmp/ftpregel" is deleted before anybody writes it in a later superstep
		//***********************************************************
		StopTimer(IO_TIMER);
	...
	void run(const WorkerParams& params) {
		...
		while (true) {
			...
			{
				message_buffer->load_rbuf();//*** FT-change-2: load msgs
				vcomp_called = false;
			}
